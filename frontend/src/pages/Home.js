...
export default Home;good*/


/*resimport React, { useState, useEffect } from 'react';
import axios from 'axios';
import { Link } from 'react-router-dom';
import Banners from '../components/banners/Banner';
import DoubleBanner from '../components/banners/DoubleBanner';
...
export default Home;res*/



/*xximport React, { useState, useEffect } from 'react';
import axios from 'axios';
import { Link } from 'react-router-dom';
import Banners from '../components/banners/Banner';
import DoubleBanner from '../components/banners/DoubleBanner';
...
const Home = () => {
    ...
    const fallbackImage = '/assets/images/ProductImages/fallback-image.png'; // Adjust the path as needed
                ...
                <div className="popular-picks-container">
                    ...
                    <div className="product-cards-container" ref={scrollRef}>
                        {products.map(product => (
                           /*1 <ProductCard
                                key={product._id}
                             image={`http://localhost:5000/${product.image}`}  id={product._id}
                              
                             // image={product.image}


                            // image={product.image ? (product.image.startsWith('http') ? product.image : `http://localhost:5000/${product.image}`) : fallbackImage}
                                name={product.name}
                                rating={product.rating}
                                reviews={product.numOfReviews}
                                price={product.price}
                   ...
                   /*2<ProductCard
             key={product._id}
          image={product.image ? `http://localhost:5000${product.image}` : fallbackImage} // Correct image handling
             id={product._id}
              name={product.name}
                rating={product.rating}
               reviews={product.numOfReviews}
...
<ProductCard
  key={product._id}
  image={product.images && product.images.length > 0 ? `http://localhost:5000${product.images[0]}` : fallbackImage} // Correct image handling
  id={product._id}
  name={product.name}
  rating={product.rating}
  reviews={product.numOfReviews}
